    ...
    try {
      ...
      if (fichaId) {
        ...
        if (
          ...
          // 🔥 CONSULTA CORREGIDA: Evalúa opciones seleccionadas y respuestas abiertas/numéricas en subpreguntas
          const [{ tiene_alertas }] = await queryRunner.manager.query(
            `SELECT EXISTS (
               ...
               WHERE r.ficha_id = $1
                 ...
                 AND (
                   (op.id IS NOT NULL AND UPPER(TRIM(op.texto_opcion)) NOT IN ('NO', 'NINGUNA', 'N/A', 'NINGUNO', 'FALSO', '0', ''))
                   OR
                   (op.id IS NULL AND UPPER(TRIM(COALESCE(r.valor_texto, r.valor_numerico::text, ''))) NOT IN ('NO', 'NINGUNA', 'N/A', 'NINGUNO', 'FALSO', '0', ''))
                 )
             ) AS tiene_alertas`,
            [fichaId],
          );
